...
- The same hooks are how the daemon knows which sessions are open: `SessionStart` registers one (labelled by the basename of the working directory), `UserPromptSubmit` renews it, `SessionEnd` removes it. A session that stops renewing expires; nothing is persisted. The list rides in every hello (§5.5) so other members see `matthew-mbp (online, 2 sessions: hivemind, scorsese)`. A session is presence, never an address (ADR 0003): mail is delivered to the node.
