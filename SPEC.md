...
- No relay through the public internet. LAN and Tailscale only.
...
Pairing with another node is a one-time fingerprint confirmation on both sides, like SSH's first connection. After that, everything is automatic.
...
├── peers.toml              # the address book (source of truth for peers)
├── mail/
...
### 5.3 Manual
- `hivemind join <host-or-ip>[:port]` — contacts the node, starts pairing (§6.2).

### 5.4 Address book policy
- A peer's address list is ordered by `last_ok`. Delivery tries each in order.
- Discovery updates addresses for known peers automatically. Unknown discovered nodes are shown in `hivemind peers` as "seen, not paired" so a user can pair with one command.
- No gossip in v1. Keep the field in the handshake reserved for it.
...
### 6.2 Pairing (TOFU)
1. A calls `POST /peer/v1/handshake` on B over TLS, presenting its client cert. B does the same in response. Both sides now have each other's cert.
2. Both daemons store the candidate in `pending_pairs`. Each user runs `hivemind pair <short-id>` (or clicks in the web UI); the CLI prints the full fingerprint and name and asks `y/N`.
3. Only after **both** sides confirm is the peer written to `peers.toml` and allowed to send mail. Until then, B rejects mail from A with `403 not_paired`.
4. A `hivemind pair --trust-network` flag exists for LANs you fully trust: auto-accepts any mDNS-discovered node. Off by default; warn loudly.

### 6.3 Transport
- rustls, TLS 1.3 only, mutual auth. No CA, no hostnames — pin by fingerprint. **Outbound** connections pin the peer's certificate from `peers.toml`, with one exception: `hivemind join` accepts what the host presents so the user can confirm its fingerprint (trust on first use). **Inbound** connections are admitted by TLS and authorised by the application: `/peer/v1/handshake` is open, everything else requires a paired peer and returns `403 not_paired` otherwise. See `docs/decisions/0010-tls-admits-strangers-the-application-rejects-them.md` — verification happens before the request line is read, so the transport cannot decide this per endpoint.
...
GET    /api/v1/peers                       paired + seen-not-paired
POST   /api/v1/peers/join      {host}      start handshake
POST   /api/v1/peers/{id}/pair             confirm pairing
...
GET    /api/v1/events                      SSE: message.received, message.delivered, peer.seen, pair.pending
GET    /healthz
...
- `SessionStart` and `UserPromptSubmit` → `hivemind hook check`: prints a one-line summary ("hivemind: 2 unread — rafael-mbp: 'dashboard PR', everyone: 'lunch?'") to stdout if there is unread mail, nothing otherwise. Must exit in < 100 ms; reads `index.db` directly, never the network.
...
hivemind join <host[:port]>
hivemind pair <short-id> [--yes]
hivemind peers [refresh|remove <id>]
...
- **Gossip** of peer lists on handshake (field reserved).
- **Windows** support.
- **Relay** node for peers not on a shared network.
